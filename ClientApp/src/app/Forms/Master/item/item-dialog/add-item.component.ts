...
export class AddItemComponent implements OnInit {
  ...
  constructor(
    ...
    private _appService: MasterService
  ) {
  ...
  initialApiCalls() {
    forkJoin([this._appService.getExchangeName()])
      .pipe(
        map((response) => {
  ...
  getItemInfo() {
    this._appService.getItemById(this.selectedId).subscribe((res) => {
      ...
      this.itemForm
        .get("lotSizeValueMultiplier")
        .setValue(res.lotSizeValueMultiplier.toFixed(2));
      this.itemForm.get("ApplyCtt").setValue(res.applyCtt);
      this.itemForm
        .get("applyRiskManagementFees")
        .setValue(res.applyRiskManagementFees);
  ...
  public onSubmit(values: Object): void {
    ...
    this.itemForm.controls["lotSizeValueMultiplier"].setValue(
      Number(this.itemForm.get("lotSizeValueMultiplier").value)
    );
    ...
    if (this.itemForm.valid) {
      ...
      this._appService.saveItem(body).subscribe(
        (result) => {
          if (result.isSuccess) {
