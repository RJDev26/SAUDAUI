...
export class AddAccountHeadComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<AddAccountHeadComponent>
  ) {
    this.selectedAccountHeadID = data.branchID || 0;
    console.log(data);
  }
  ...
  onGridReadySelectAc(event) {
    this.gridApiSelectAc = event.api;
  }

  getAcHeadAccountList() {
    this._masterService
      .getAccountsAddedinAcHead(this.selectedAccountHeadID)
      .subscribe((res) => {
  ...
  getAccounts() {
    this._masterService
      .getAccountsForBranch(this.selectedAccountHeadID)
      .subscribe((res) => {
  ...
  getApiData() {
    if (this.selectedAccountHeadID) {
      this._masterService
        .getAcHeadId(this.selectedAccountHeadID)
        .subscribe((res) => {
  ...
  public onSubmit(values: Object): void {
    ...
    if (this.nameForm.valid) {
      ...
      this._masterService.saveAccountHead(body).subscribe(
        (result) => {
          if (result.isSuccess) {
