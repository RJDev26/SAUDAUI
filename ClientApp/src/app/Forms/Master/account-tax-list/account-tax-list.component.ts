...
@Component({
  selector: 'app-account-tax-list',
  templateUrl: './account-tax-list.component.html',
  styleUrls: ['./account-tax-list.component.scss']
})

export class AccountTaxListComponent implements OnInit {
    ...
    downloadAsPDF() {
      ...
      const mainHeader = "Account Tax List";
      ...
      doc.save("AccountTaxList.pdf");
    }

    getExchangeTaxList() {
        this._masterService.getAccountTaxList().subscribe((results) => {
          debugger
          if (results.isSuccess) {
            this.accountTaxList = results.data;
          }
